This function Verifies the specified in-toto [envelope].
...
```
The `blob` (binary large object) is the artifact for the `digest` in the
...
This data can then be passed onto other pattenrs for further evaluating rules
...
[envelope]: https://github.com/in-toto/attestation/blob/main/spec/v1.0-draft/envelope.md
